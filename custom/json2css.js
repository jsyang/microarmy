...
function json2css(input, options) {
  ...
  inputObj.strip();
  // jsyang-remove: inputObj.ensureOffsetAndPx();  
  // jsyang-remove: inputObj.escapeImages();
...
Input.prototype = {
  // jsyang-add: Remove all unneeded JSON properties
  'strip': function() {
    this.input.forEach(function(item){
      delete item["source_image"];
      delete item["image"];
      delete item["total_width"];
      delete item["total_height"];
      delete item["escaped_image"];
    });
  },
